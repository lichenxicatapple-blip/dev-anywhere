...
| permission broker  | 权限状态 owner    | shared schema、session registry                       | UI 组件、provider 私有 transport          | pending permission、allow/deny/drain/delivered                 |
...
| permission request  | provider -> proxy -> relay -> client      | permission broker | 是，pending 列表     | 不可丢                 | 是，业务 ack/result  | `tool_use_request`      |
| permission decision | client -> relay -> proxy -> provider      | permission broker | 否                   | 不可丢                 | 是，delivered/result | `tool_approve/deny`     |
...
| permission state      | permission broker  | pending、delivered、decision、drain；审批无限等待，不做时间自动拒绝      |
...
| PTY idle timer           | 已降级   | 否                     | 仍可产生 `pty_state` 观察事件，但不再写主状态          |
| OSC approval/working     | 已降级   | 否                     | 仍可产生 `pty_state` 观察事件，但不再写主状态          |
...
4. **降级 `pty_state`**：`pty_state` 保留给短期 UI 提示或调试，不再写入 `session.state` 主路径。web/proxy 主路径已完成。
...
1. 产出 `docs/governance/PROTOCOL-STATE-GOVERNANCE.md`，列出现有消息、owner、方向、状态影响、删除债务。
...
6. 删除或降级 proxy `PtyObserver` 的主流程状态写入。已完成。
