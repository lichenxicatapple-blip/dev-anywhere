...
A *profile* is a saved selection of which *relay* the proxy daemon should connect to, plus profile-scoped runtime state (PID, socket, logs). A *relay* is a saved (URL, proxy-token) pair. Multiple profiles can point at the same relay, and the proxy daemon for each profile runs in its own isolated workspace.
