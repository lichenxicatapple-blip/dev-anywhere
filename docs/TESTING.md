...
| **L4 mobile** | `pnpm test:mobile` | 真 Android emu + Chrome over CDP: 真触屏/IME/visualViewport/几何 | emulation 已能复现的 layout (归 L2) | ~6 min |
...
pnpm test:mobile                          # L4 真 Android emu; 默认最多 2 shard (没 emu 自动 skip 退 0)
...
# mobile emulator pool:
pnpm test:mobile:emulators:start -- 2     # 启动/复用 2 台 DEV Anywhere 专用 emulator
pnpm test:mobile:emulators:list           # 查看 AVD 和在线设备
pnpm test:mobile:emulators:stop -- 2      # 停掉专用 emulator
```
...
`pnpm test:mobile` 由 `scripts/test/mobile-parallel.sh` 分发到在线 emulator。默认最多使用 2 个 worker, 因为实测 4 个 emulator 会共享宿主机 CPU/GPU/adb/Chrome 启动资源, 更容易触发 CDP ready 或触摸时序 flake; 3 个 worker 在当前机器上也没有比 2 个更快。要压测并发上限时显式设置:

```bash
TEST_MOBILE_PARALLEL_WORKERS=3 pnpm test:mobile
TEST_MOBILE_PARALLEL_WORKERS=4 pnpm test:mobile
```

`pnpm release:smoke` 会自动启动或复用 2 台 DEV Anywhere 专用 emulator, 并用 `TEST_MOBILE_REQUIRE_EMULATOR=1` 跑 mobile gate, 所以正式 release 不会因为没有 emulator 而静默 skip。日常开发仍推荐让 emulator 常驻, 避免每次手动跑 mobile 时冷启动:

```bash
pnpm test:mobile:emulators:start -- 2
pnpm test:mobile
```

### Mobile 边缘滚动诊断

release 默认 L4 gate 不再跑最脆的完整手势链路: "长按选区后拖动终点到底部边缘触发 autoscroll 并复制"。这个场景依赖 Android emulator 的触摸调度、Chrome Pointer/Touch 事件转换、RAF 和滚动节奏; 在宿主机负载抖动时容易 flaky, 不适合作为发布必过卡点。

默认 mobile gate 会用 DEV-only selection driver 构造多行选区, 专注验证选区后的 toolbar、复制、下载和清空行为; 它不代表完整 Android 手势链路已经通过。

没有删除产品能力覆盖:

- `apps/web/src/components/chat/use-pty-touch-gesture.test.tsx` 覆盖长按状态机、拖动、pointer/touch 混合和 pointercancel。
- `apps/web/src/components/chat/use-pty-selection-gesture-driver.test.tsx` 覆盖选区 handle 拖动和 native scroll suppression。
- `apps/web/src/lib/pty-edge-autoscroll.test.ts` 覆盖边缘滚动方向和边界。
- `apps/web/src/lib/pty-drag-select-autoscroll.test.ts` 覆盖拖选 autoscroll 事件派发。
- `apps/web/e2e/mobile/pty-long-press-copy.spec.ts` 默认仍覆盖多行复制、短 token 长按、文件链接下载、选区 toolbar 和清空行为; 最脆的边缘自滚诊断用例默认 skip。

需要专项验证选区 handle 边缘自滚时, 显式打开诊断:

```bash
TEST_MOBILE_EDGE_AUTOSCROLL_DIAGNOSTICS=1 bash scripts/test/mobile.sh e2e/mobile/pty-long-press-copy.spec.ts
```

改动触及 PTY 选区、长按、选区 handle、边缘 autoscroll、软键盘关闭后的选区定位时, 发版前至少手动在 Android emulator 或真机确认:

- 长按 PTY 文本能出现复制 toolbar 和起止 handle。
- 长按文件路径能选中完整路径, toolbar 里下载入口可用。
- 拖动选区终点到视口底部边缘时, 终端能持续向下滚动。
- 软键盘收起后, 选区 toolbar 和 handle 仍锚定在正确文本附近。
