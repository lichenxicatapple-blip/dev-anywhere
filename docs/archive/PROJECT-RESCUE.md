...
当前产品名确定为 **DEV Anywhere**，详见 `docs/archive/NAMING-MIGRATION.md`。新定位不是只服务 Claude Code，而是 **本地 AI CLI 透明代理 + 可选远程镜像**。首批支持 Claude Code 和 Codex；用户的日常入口应该是 `dev-anywhere` 透明代理命令，体验上尽量等价于原始 CLI（例如 `claude [args...]` 或 `codex [args...]`），但额外把本地会话桥接到 relay/web。
...
详细审计见 `docs/research/TECH-DIFF-LINKSHELL.md`。本节只保留救援计划需要的摘要。
...
| PTY 远端输入     | Web 主入口曾是聊天式 `InputBar`，Enter 后把整段文本写入 PTY                                         | Web 发 `terminal.input` 原始输入，gateway 转 host，host 直接 `pty.write(data)`       | 必须补逐键 raw input，PTY 模式不再保留聊天框 |
...
- hook server 嵌入 `dev-anywhere serve`，固定监听 `127.0.0.1:<配置端口>`；不做独立全局 daemon，不默认随机端口。
- `serve` 为每个 session 生成 `sessionId + marker + token`，provider hook 回调时必须携带这些凭据，权限响应通过挂起的 hook request 返回 provider，不绕回 terminal runtime。
...
- 全局写入不是常规路径。只有当 provider 官方能力确认不存在进程级、会话级、项目级注入，并且用户显式执行 setup 时，才允许作为单独安装模式。
- Claude Code 全局写入目标可能是 `~/.claude/settings.json`，但必须走显式 setup，不允许运行时静默修改。
- Codex 全局写入目标可能是 `~/.codex/hooks.json` 和 `~/.codex/config.toml`，但必须走显式 setup，不允许运行时静默修改。
...
- 权限请求有 pending map、delivered/drain 和 resolution result，并且必须无限等待用户决策；DEV Anywhere 不因时间流逝自动拒绝。provider hook 如果要求有限 timeout，只能设置极长 lease 来规避默认短超时。
...
11. 计划中禁止把旧实现包装成“备用方案”。备用路径只能用于启动失败保护、网络断线恢复、用户显式选择的安装模式；其他旧路径必须写删除条件和删除时间点。

## 命名优先级

项目改名是第一优先级。旧名 `CC Anywhere` 已经不适合新定位，因为产品不再只围绕 Claude Code，而是要成为 Claude Code、Codex 等本地 AI CLI 的透明代理和远程镜像层。

命名迁移细节见 `docs/archive/NAMING-MIGRATION.md`。新产品名确定为 **DEV Anywhere**，主命令为 `dev-anywhere`。
...
- 第一版 `docs/research/TECH-DIFF-LINKSHELL.md` 已创建，明确了必须追赶、可以清理/重写、不建议照搬三类差异。
...
- 第一版 `docs/archive/NAMING-MIGRATION.md` 已创建。
...
- 第一版 `docs/governance/ARCHITECTURE-GOVERNANCE.md` 已创建。
- 模块 owner、协议消息分类、状态来源规则、provider adapter contract、hook 注入策略和 review checklist 已写入。
- 第一版 `docs/governance/PROTOCOL-STATE-GOVERNANCE.md` 已创建，明确协议分层、状态 owner、LinkShell 差距、删除债务和第一阶段实施顺序。
- shared 已新增 provider-neutral `agent_status` / `agent_status_request` contract，relay 控制消息方向从 shared 元数据派生。
- proxy 已维护当前 `agent_status` registry，web 在冷启动、重连、切 proxy、进入 chat 时按需拉取当前状态；relay 不缓存 provider 语义状态。
- JSON approval pending 已合并进统一 permission broker，`ToolApprovalManager` 已删除；`worker_approval_request/response` 只保留为 worker IPC transport。
- permission 已补 `request_delivered` / `decision_result` 语义：web 收到请求后 ack，proxy resolve 后回结果，UI 不再只靠本地乐观成功。
...
- 按 `docs/governance/ARCHITECTURE-GOVERNANCE.md` 创建第一轮 provider adapter contract。
- 按治理边界拆出 hook server 和 permission broker。
- 定义 `serve.ts` 拆分落点：启动装配留在入口，路由/hook/permission/session lifecycle 拆出。
- 按 `docs/governance/PROTOCOL-STATE-GOVERNANCE.md` 继续收紧 provider status / permission / lifecycle 分层，禁止重新引入 PTY 状态猜测主路径。
...
- 用 mock hook event 验证 tool status、permission request、allow/deny、无限等待和退出清理。
...
- PTY 远端输入改为逐键 raw input 模型；聊天式 `InputBar` 仅用于 JSON 模式，PTY 模式不再渲染。
...
- Proxy agent status request tests。
- 手动断线/重连 smoke。
