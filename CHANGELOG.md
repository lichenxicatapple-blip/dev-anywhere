...
- Cursor CLI 支持 ACP 聊天会话：流式输出、工具调用与审批、提问、计划批准、todo 列表，以及取消当前回合。恢复时先走 `session/load`，失败则新建会话并提示。

## [0.9.16] - 2026-09-13

### 改进

- 文件选择器新增“磁盘/挂载点”，支持切换 Windows 盘符以及 macOS、Linux 的挂载目录。
- 路径输入框支持直接输入或粘贴路径，也可以从输入的目录继续浏览。

### 修复

- 修复终端长按网址时只选中域名的问题。现在会包含路径、参数和片段，支持自动换行的网址，以及 FTP、SFTP 等常见协议的地址。

## [0.9.15] - 2026-09-13

### 修复

- 修复 Kimi Code 等终端会话在手机键盘弹出后，画面随界面刷新反复上下跳动的问题。

## [0.9.14] - 2026-09-12

### 修复

- 修复部分 Node.js 环境中，后台服务或 Quick Tunnel 无法启动，以及终端组合字符、emoji 宽度计算异常的问题。

### 升级说明

- 已有会话会保留；若会话仍有 emoji 错位，重新创建会话后生效。

## [0.9.13] - 2026-09-12

### 改进

- 按 Claude Code、Codex、Kimi 和实际使用的 Shell 提供终端快捷键。移动端优先显示常用操作，保留各 Agent 的一键清空，更多快捷键可从会话菜单发送。
- 调小手机端终端行数、列数的常态字号，与菜单文字保持协调。

### 修复

- 修复 Kimi 等终端界面刷新时，画面反复左右跳动、闪烁的问题。
- 手动横向滑动查看长行后，后续输出会保留查看位置；继续输入时恢复光标跟随。
- 修复持续输出、自动滚底时，垂直滚动条反复出现的问题；触摸或鼠标操作时仍可显示和拖动。

### 升级说明

- Shell 专属快捷键在升级后新建的会话中生效；已有会话可继续使用。

## [0.9.12] - 2026-09-12

### 改进

- 托管终端的行数、列数支持直接点击数字输入，保留加减按钮。输入后按回车或移开焦点确认，Esc 取消；只接受范围内的正整数。
- 会话菜单中的“快捷键”改为“发送快捷键”，再次点击可收起二级菜单并保留主菜单。

### 修复

- 修复 Windows PowerShell、PowerShell 7 和 CMD 切换目录后，下载相对路径文件仍使用旧目录，导致找不到文件或下载到旧目录同名文件的问题。
- 恢复 Windows Shell 切换目录后，会话列表和顶部目录标题的同步更新；手动设置的会话名称会保留。
- 修复 Windows 反斜杠路径未被识别为文件下载链接的问题，并完善图片路径和选中路径的识别。
- 修复目录变化信息分段到达时可能丢失更新，以及 Windows 盘符、网络共享路径解析不正确的问题。

### 升级说明

- Windows Shell 目录同步对升级后新建的会话生效。已有 Shell 会话会保留，需要重新创建后才能使用这项修复。

## [0.9.11] - 2026-09-12

### 新功能

- 会话菜单新增独立的“终端尺寸”分组，列数、行数可查看当前值并逐次增减，保留“按窗口调整终端尺寸”。调整后，连接同一会话的其他浏览器会同步更新。

### 改进

- 将终端快捷键收进二级菜单，缩短主菜单，并让手机上的子菜单保持在屏幕内。
- 收紧菜单项、箭头和右侧留白，保持菜单紧凑，并避免菜单展开时菜单项移位。
- Codex 终端会话隐藏不可用的 Always yes 选项。
- 补充 macOS 文件夹访问授权，以及 FileVault 对未登录启动的影响说明。

### 修复

- 修复 macOS 后台环境中，本地开发启动脚本未能启动 Relay 和 Web 的问题。

## [0.9.10] - 2026-09-11

### 修复

- 修复 Windows 自动更新被旧安装目录中的占用文件阻塞、重试仍无法完成的问题。
- 修复 Windows 系统服务已运行时，普通终端查询状态或执行启停命令却报权限错误的问题。

### 改进

- 发布前必须通过 macOS、Linux 和 Windows 的自动升级验收，覆盖普通后台、系统服务、下载超时后的自动重试和已有 Shell 保留。

### 升级说明

- 已启用 Windows 系统服务的用户，升级完成后需在管理员 PowerShell 中执行一次 `Restart-Service -Name '服务名称'`，之后即可在普通终端查询状态和启停。服务名称可用 `dev-anywhere serve autostart status --system` 查看。

## [0.9.9] - 2026-09-10

### 修复

- 修复 Windows 本机账户启用、启动或取消系统自启动时报账户识别错误的问题。
- Windows 服务启动失败时显示具体错误码，并在账户登录失败时提示检查密码和登录权限。
- Windows 系统服务首次安装时验证账户密码，密码错误可当场重新输入。
- 自动更新失败时从本地恢复原版本和命令入口，修复 npm 下载超时后残留文件导致后续重试持续失败的问题。

## [0.9.8] - 2026-09-10

### 新功能

- 新增可选的系统服务模式，支持 macOS、systemd Linux 和 Windows 在无需桌面登录时启动 Proxy，并以原用户身份运行新建的 Shell 和 Agent 会话。
- 新增 `serve autostart enable --system`、`status --system` 和 `disable --system`；启用时加 `--now` 可立即启动系统服务并重启 Proxy。

### 改进

- 系统服务模式支持原有的启停命令和自动更新。
- 切换自启动模式时自动取消旧设置；不同 profile 可分别设置。

### 修复

- 修复 Linux 登录自启动可能因工作目录解析错误而无法启动的问题。
- 修复部分浏览器中聊天输入栏与消息右侧未对齐的问题。

### 升级说明

- 系统服务为可选功能，原有登录自启动保持可用。安装系统服务需要管理员授权，Windows 首次安装还需要原用户的账户密码。
- 退出桌面后需要继续使用的会话，请在启用系统服务后新建。配置、验证与取消步骤见[系统服务指南](./docs/SYSTEM-SERVICE.md)。

## [0.9.7] - 2026-09-10

### 新功能

- Windows 开发机新建 Shell 时，可在独立的命令行类型选择页选择 PowerShell 或 CMD，并显示所选终端的创建进度。
- 会话菜单新增发送 Ctrl+R；Codex 终端会话新增发送 Alt+↑、Alt+↓ 和 Ctrl+]，方便手机操作异步问答等终端界面。

### 修复

- 修复 Windows 终端持续输出时，网页视口可能随临时光标位置反复横向抖动的问题。
- 修复 Windows Codex 终端中网页 Shift+Enter 无法换行，以及混合使用本地和网页输入后 Esc 可能失效的问题。
- 修复关闭会话菜单后终端失去键盘焦点，以及部分终端会话菜单未显示 Ctrl+O 的问题。
- 修复 Codex 普通提问被误判为等待审批、持续输出反复触发自动回车并误提交草稿的问题；Codex 终端会话停用 Always yes 自动回车，权限提示需手动处理。
- 修复 Windows 用户目录简写路径 `~\` 指向的图片和文件无法打开的问题。
- 修复浏览器异常断开可能导致本地开发服务退出的问题。
- 修复 Windows 在线安装器通过 SSH 部署时，控制台编码可能干扰远端脚本的问题。

### 改进

- 通过 DEV Anywhere 启动或恢复 Codex 终端会话时关闭星芒特效，减少终端显示干扰。
- 本地开发的启动、重启和健康检查命令支持直接在 Windows PowerShell 或 CMD 中运行。

### 升级说明

- 已在运行的终端进程不会热加载 Windows 终端兼容修复。升级后，请在合适的时机退出并重新启动或恢复相关会话；仅刷新网页或重启 Proxy 不会更新这些进程。

## [0.9.6] - 2026-09-07

### 新功能

- 网页和模拟器预览支持在后续更新或重启 Proxy 后继续使用，保留预览名称和网页分享链接。

### 改进

- 统一路径选择器、历史会话和预览列表中的路径显示，用 `~` 简写开发机的用户目录，支持 macOS、Windows 和 Linux 路径。

### 修复

- 修复重新连接开发机后，预览列表首次加载失败时无法自动恢复的问题。

### 升级说明

- 首次升级至本版时，原有网页预览需要重新连接，模拟器预览需要重新创建。之后正常更新或重启 Proxy 不再丢失预览；Agent 会话不受此次调整影响。

## [0.9.5] - 2026-09-06

### 新功能

- 托管终端会话新增“按窗口调整终端尺寸”，切换手机或电脑后可在会话菜单中重新调整显示尺寸。

### 改进

- 在开发机名称下方显示操作系统，便于区分 macOS、Windows 和 Linux 开发机。
- Windows 开发机的用户目录支持用 `~` 简写显示。

### 修复

- 修复 Windows 浏览器中按 `Ctrl+V` 粘贴文字时，可能误触发远端 Codex 图片粘贴的问题。
- 修复聊天视图读取部分 Codex 历史记录时遗漏用户消息和回复的问题。
- 修复 Windows 登录自动启动后残留空白 PowerShell 窗口的问题。
- 修复 Windows 上停止托管终端会话后，后台进程可能无法退出的问题。
- 修复 Always yes 图标容易被误认为已开启，以及网页预览地址与状态文字间距过大的问题。

### 升级说明

- Windows 用户如已开启自动启动，请在更新后执行一次 `dev-anywhere serve autostart enable`，使静默启动生效。

## [0.9.4] - 2026-09-06

### 改进

- 调整通信协议的扩展规则，避免后续新增的非必需信息导致连接中断或状态同步失败。
- 保留消息转发中的附加信息，避免不同版本之间传递信息时被中转服务丢弃。

## [0.9.3] - 2026-09-05

### 新功能

- 托管 Agent 会话支持在后续更新或重启 Proxy 后继续使用。

### 修复

- 修复历史会话列表混入 Agent 内部子任务、空记录或损坏记录的问题。
- 修复同目录下同名的独立会话被错误合并的问题。
- 修复历史会话工作目录识别不准确的问题。
- 修复部分会话重命名后，历史列表名称未同步的问题。

## [0.9.2] - 2026-09-05

### 新功能

- 支持将 Windows 作为开发机，运行 Agent CLI 和 Shell 会话。
- 新增登录后自动启动选项，支持 macOS、Linux 和 Windows，可通过命令行开启或关闭。

### 修复

- 修复版本不匹配时持续重连、反复刷日志的问题，避免长时间占用资源。
- 修复 Proxy 启动、停止和升级重启可能相互干扰的问题，避免重复启动或停止后被再次拉起。
- 修复手机端离线开发机的“移除”按钮提前露出、与列表项错位的问题。

### 升级说明

- 从此前版本升级时，需要先用原来的命令行停止 Proxy，再更新 Relay、安装新版 Proxy 并启动。本次升级会结束旧版终端会话，更新后需重新启动。具体命令见 [升级步骤](./docs/DEPLOYMENT.md#升级)。

## [0.9.1] - 2026-09-04

### 修复

- 修复升级 Relay 后，已启用自动升级的 Proxy 可能无法获取新版本并持续离线的问题；Proxy 现在会自行完成更新并重新连接。

## [0.9.0] - 2026-09-04

### 新功能

- 新增网页预览，可在浏览器中查看开发机上运行的网站、HTML 文件或网页目录，并通过 Cloudflare Tunnel 或 Cpolar 生成临时 HTTPS 链接。
- 新增移动设备模拟器预览，可在浏览器中查看和基本操控开发机上运行的 iOS Simulator 或 Android Emulator。

### 改进

- 统一工作目录、Agent CLI 可执行文件和网页文件的选择界面。

### 修复

- 修复调整终端窗口大小时会重新连接并重放整屏内容的问题，调整尺寸时会话现在保持连接。
- 修复 Kimi Code 等终端界面在高延迟网络下反复清屏或长时间停留在中间画面的问题。
- 修复关闭“设置”子页面时短暂闪回设置菜单的问题。
- 修复 Relay 暂时不可用时误显示没有开发机且页面无法自动恢复的问题。

### 升级说明

- DEV Anywhere 0.9.0 不兼容此前版本。升级 Relay 后，请在每台开发机上手动更新并重启 DEV Anywhere；随后刷新浏览器，并重新启动升级前仍在运行的会话。

### 文档

- 更新 README 图片并补充预览功能说明。

## [0.8.1] - 2026-09-01

### 修复

- 修复 Relay 自动升级重启 Proxy 时，新 daemon 继续继承旧进程 `PATH`，导致仍然无法识别升级前新安装的 CLI 的问题。自动更新现在会在停止旧 daemon 之前从用户的 POSIX 兼容交互式 login shell 读取最新 `PATH`，只替换该字段并保留其他环境变量；读取失败或超时时安全回退到原 `PATH`，不阻断服务恢复。
- 手动执行 `dev-anywhere serve start` 或 `dev-anywhere serve restart` 仍保持现有语义，直接继承当前调用终端的环境，不会被自动更新的刷新逻辑覆盖。

### 测试

- 增加 login shell 噪声输出、超时、输出上限与 `PATH` 校验，以及自动更新父进程识别、手动/自动重启环境隔离和失败回退回归。
- 使用真实父子进程、隔离 `HOME` 与临时自动更新锁验证首次由旧 updater 启动新版 CLI 时也会刷新 `PATH`，且不会触碰当前 daemon。

## [0.8.0] - 2026-09-01

### 新功能

- 新增 Kimi Code 终端与 ACP 聊天会话：支持 `dev-anywhere kimi ...` 接管本地 CLI，也可以从 Web 在指定工作目录创建由 Proxy 托管的 PTY 或 `kimi acp` 聊天会话。
- 新增 Kimi Code CLI 自动探测、`KIMI_BIN` 临时覆盖、`agentCli.kimiBin` 持久路径与路径历史；Web 可以查看和修改实际使用的可执行文件。
- Kimi ACP 聊天支持流式输出、工具调用与交互审批、取消当前回合和恢复历史会话；ACP 提供动态审批选项时，Web 会按原始名称展示并回传精确选项，普通审批继续复用允许、始终允许与拒绝操作。
- Web 提供手工审批、自动审批、只读规划和全自动四档 Kimi 权限模式。PTY 分别使用原生默认行为、`--yolo`、`--plan` 与 `--auto`，ACP 会话分别配置为 `default`、`yolo`、`plan` 与 `auto`；PTY 恢复使用 `--session <id>`，ACP 通过 `session/resume` 恢复并沿用分页历史读取，避免加载长会话时全量回放。
- 支持手动移除 Relay 中不再使用的离线开发机：手机端左滑露出移除操作，桌面端通过开发机行的更多菜单操作，并在确认框说明机器以后重新连接时仍会再次出现。在线开发机不会显示移除入口；确认期间抢先上线的开发机会由 Relay 拒绝移除。

### 兼容性

- Kimi ACP 聊天使用原生 ACP 事件与审批协议，不注册 Claude/Codex hooks；移动端 Kimi 终端的清空输入使用带防重复保护的 Ctrl-C。新版 Web 仍可读取未上报 Kimi 状态的旧 Proxy。

### 测试

- 增加 Provider 参数映射、CLI 探测与配置、IPC、托管与本地 PTY、ACP 流式事件、工具审批、取消与恢复、会话持久化、Web 创建流程和移动端控制回归；使用本机 Kimi Code 0.39.1 验证终端四档参数与 ACP 链路。
- 增加离线开发机移除的协议校验、权限与状态竞态、跨标签页绑定清理、远程文件令牌撤销、手机侧滑、桌面菜单、二次确认及删除后重新连接回归。
...
- 修复手机打开已有长行输出的 PTY 会话时，恢复快照会被误认为正在输入的实时内容，导致页面初始水平位置自动向右偏移的问题；恢复会话现在保留左侧阅读起点，只有光标真正离开可见区时才会紧急跟随。
- 保留移动端长命令实时输入时的提前水平跟随，光标接近屏幕右边缘时仍会预留输入空间，不会因修复快照恢复而退化。

### 测试

- 增加 Android Chrome 回归门禁，同时验证恢复快照保持左侧锚点、实时输入继续跟随，并稳定菜单动画期间的桌面布局几何校验。

## [0.6.2] - 2026-08-27

### 修复

- 修复升级或重启 Proxy 时旧 daemon 尚未完全退出，新 daemon 就开始启动，导致仍在运行的本地终端会话失去登记、Codex 原生会话继续持有 writer lock 却无法从 DEV Anywhere 打开的问题；现在会等待旧进程确实退出，并确认存活的本地终端已在新服务完成交接，失败时明确报错而不会静默宣告成功。
- 修复恢复一个仍被其他 Codex 进程使用的会话时只显示笼统启动失败的问题；属于当前 DEV Anywhere 的同一会话会直接复用并进入，外部终端、Codex App 或其它进程占用时则显示锁持有 PID 与处理方法，且不会自动终止用户进程。
- 修复新版 Codex 已移除旧 `untrusted` 审批策略后，DEV Anywhere 仍把“严格审批”映射到旧协议而导致恢复失败的问题；Codex 现在只提供“按需审批”和“跳过全部审批”，旧页面发送的不支持策略会在 Proxy 协议入口被明确拒绝，不再静默降级。
- 修复 Provider 非零退出时关键错误随临时进程结束而丢失的问题；本地 PTY、托管 PTY 与 JSON worker 现在都会保留一小段经过 ANSI 清理、主目录替换和常见凭据脱敏的错误尾部，并把可识别的 Codex writer 冲突结构化回传给页面。

### 测试

- 增加 Codex 审批策略、当前会话直接复用、外部 writer PID 探测、PTY 与 JSON 启动竞态、错误尾部脱敏、daemon 退出等待、本地终端交接、Web 冲突弹窗及协议往返的回归门禁。

### 升级

- 本版增加会话占用错误码与运行时错误消息；滚动部署时先升级 Relay/Web，再升级并重启 Proxy，最后刷新仍打开的旧页面。

## [0.6.1] - 2026-08-21

### 修复

- 修复手机连续刷新或多个页面同时加载历史会话时，Proxy 会并发重复扫描全部 Claude/Codex 历史文件，造成磁盘扫描风暴、响应从正常约 2 秒延长到 30 秒以上并最终触发页面超时的问题；重叠请求现在会在 Relay 与 Proxy 两层合并，失败或卡住的请求仍可按时重新发起。
- 修复历史会话响应被 Relay 广播给同一开发机上的所有浏览器、放大带宽和解析开销的问题；每个响应现在只交付给仍在线的原请求页面，页面断开、Proxy 重连、迟到响应与请求 ID 冲突均不会串流或污染其它页面。

### 改进

- 35 个重叠历史请求现在只产生 1 个 Relay 到 Proxy 请求、1 次磁盘扫描和 1 个 Proxy 到 Relay 大响应；参考历史数据下，Proxy 到 Relay 的压缩传输量约从 504 KiB 降至 14.4 KiB，减少约 97%。
- 历史会话请求与响应现在强制携带 `requestId`，不符合协议的消息会在入口直接拒绝，不再保留无 ID 广播兼容路径。

### 升级

- 本版同步更新历史会话请求协议；部署时必须先升级并重启所有 Proxy，再升级 Relay/Web 并刷新旧页面。

### 测试

- 增加 35 路刷新风暴、两层 singleflight、精确响应路由、请求失败扇出、页面断开、Proxy 重连、飞行窗口轮换、迟到响应、容量上限及缺失请求 ID 拒绝的回归门禁。

## [0.6.0] - 2026-08-21

### 修复

- 修复首页活跃会话先返回、历史扫描仍在进行时，手机与桌面会错误显示“暂无会话记录”的问题；历史会话现在拥有独立的加载、成功与失败状态，只有开发机明确返回空结果才展示空态。
- 修复没有已保存开发机时从会话深链首次绑定，返回首页后不会自动请求历史会话、只能手动刷新的问题；扫描失败或超时会解锁刷新并显示可重试状态，刷新失败时继续保留已有结果。
- 修复移动端终端选区手柄与临时滚动条重叠时，触摸手柄可能被误识别为点击滚动条、导致画面大幅跳动并丢失选区的问题。

### 改进

- 历史会话改为不阻塞活跃会话的异步加载：加载期间显示明确占位并禁用重复刷新，数据到达或请求超时后自动恢复交互。
- 创建 PTY 前按需预加载常用 Powerline 分支与分隔符字形，避免提示符首次出现时使用 fallback 字形或宽度跳变；字体分片不会增加普通列表与 JSON 会话页面的 Relay 流量。

### 升级

- 本版同步更新历史会话响应协议；单机部署需先升级并重启 Proxy，再升级 Relay/Web 并刷新旧页面。

### 测试

- 增加延迟历史响应、显式空结果、扫描失败、超时、重复刷新、开发机切换与倒序响应的状态回归，并在桌面 Chromium 和 Android Chrome 中验证加载占位、活跃会话可用、44px 触控目标及数据到达后的自动解锁。

## [0.5.47] - 2026-08-21

### 修复

- 修复进入或重连长 PTY 会话时，全量快照广播给同一开发机上的其他浏览器、慢响应又触发重复快照，导致 Relay 带宽放大和同步长时间排队的问题；快照现在按请求精确回送并去重重试，只有快照与连续实时尾帧真正写入、绘制完成后才结束同步。
- 移除桌面与移动端 PTY 向上回看时的冻结画面；新输出不再抢走阅读位置，可见的 `Working...` 等状态继续实时更新，向下滚动会逐步经过期间新增的内容，到达真实尾部后才恢复跟随，回到底部按钮和输入仍可显式返回最新画面。
- 修复实时回看期间在触摸惯性、短终端补画、软键盘或字体重排、scrollback 淘汰、终端 buffer 切换及重连重置下可能出现的空白、跳行，以及选区高亮、手柄和复制内容错位；选区现在始终依附当前实际绘制的行，跨 buffer 重置会安全清除。
- 修复 PTY 查找命中位于实时补画区域时，结果计数正确但当前匹配项没有可见高亮的问题。
- 修复新建 JSON 会话时，延迟到达的初始历史响应可能覆盖刚发送的问题和已经完成的实时回复，导致消息从界面消失的问题。

### 改进

- 对大型 JSON 快照启用有界 WebSocket 压缩，同时让高频 PTY 二进制帧保持不压缩，并合并快照解析产生的大量滚动协调与批量写入，降低 Relay 带宽、浏览器主线程占用和长会话同步延迟。

### 测试

- 新增长会话全链路基准与正确性门禁，覆盖 1/3/5 个浏览器共享 300 KiB/s 下行、持续实时输出、快照精确路由、压缩后线速、乱序/缺帧和最终绘制；参考环境中单客户端由 4.580 秒降至 1.616–1.634 秒，三客户端由 45.672 秒降至 3.209–3.628 秒，五客户端由超过 300 秒未完成降至 5.784–5.883 秒。
- 增加桌面 Chromium 与 Android Chrome 的实时回看回归，覆盖浅上滚、持续输出锚定、向最新内容逐步滚动、短终端、触摸惯性、软键盘、选区与显式回到底部。

## [0.5.46] - 2026-08-21

### 修复

- 重构 PTY 桌面与移动端选区，消除 xterm 私有滚动、外层滚动和合成鼠标事件同时改写选区的多重 owner；修复静止按住即滚动、只滚动不选中、起点漂移、释放后选区继续变化等问题，并支持上下左右跨页拖选、滚动保留选区与显式点击清除。
- 将移动端选区手柄放入终端内容的 compositor 滚动层，滚动中不再追帧漂移；复制菜单在滚动时隐藏，停止后依当前可见端点重新锚定，选区离开视口时保持隐藏。
- 修复手机切到后台、锁屏、软键盘变化或多指操作后选区手势和自动滚动可能残留，以及拖动手柄首帧跳动、宽字符边界复制错位的问题。
- 修复 Linux/Windows 上已有托管选区时 `Ctrl+C` 被误发为远端 SIGINT，导致复制失败和视图跳到底部；同时保留 macOS `Ctrl+C` 的终端中断语义，并修复局域网 HTTP 降级复制后终端焦点丢失。
- 修复 PTY 查找 Next/Previous 因清除 xterm 原生搜索游标而反复停在第一项的问题。
- 修复 xterm DOM renderer 首次加载 `›`、`·`、`•` 字体分片时缓存 fallback 字宽，导致终端输入区右侧出现约 3px 背景缺口；字体延迟就绪后会正确失效宽度缓存。
- 修复历史投影滚动后 dim truecolor 前景变淡，以及冻结行在 scrollback trim 后的选区位置、高亮与复制内容可能错位的问题。
- 修复手机前台断网形成半开 WebSocket 后，信号恢复仍无法输入且必须刷新页面的问题；新增前台验活、online 强制换链和 Proxy binding 有界重试。
- 修复 PTY 初始化或重连完成时自动聚焦终端、抢走用户已打开菜单或输入控件焦点的问题。

### 测试

- 使用真实 Chromium 鼠标与 CDP 触摸门禁替换误导性的合成选区测试，覆盖静止按压、四向与跨页拖选、释放冻结、滚轮、手机滚动保留、手柄、复制菜单、真实剪贴板与远端输入隔离。
- 增加前台半开连接、断网恢复、字体加载时序、历史投影、buffer row identity 与选区复制语义的确定性回归。

## [0.5.45] - 2026-08-20

### 修复

- 修复桌面端从下向上拖选 PTY 历史文本时，xterm 私有视口自行滚动、外层页面却保持不动，导致选区内容与未选内容发生视觉错位的问题；选区自动滚动现在统一交给外层历史投影处理，移动端触摸选区沿用同一套几何语义。
- 修复连接或重连开发机时，多个常驻 PTY 同时销毁重建并重复解析未变化快照，长时间占用浏览器主线程，连带造成输入、滚动和打字机动画卡顿的问题；终端实例现在跨连接中断保留，后台会话分帧恢复，只有服务端状态实际变化时才执行完整权威快照恢复。
- 修复重连快照恢复可能与 xterm 尚未处理完的旧写入交错的问题；恢复前现在等待终端写队列屏障，恢复期间到达的实时帧继续按序缓存并在快照完成后重放，优先保证终端状态正确。

### 改进

- 保持所有已访问且仍存活的 PTY 会话常驻，不恢复三个会话的容量上限；当前会话立即恢复，后台会话逐帧接入，兼顾多会话连续性与前台交互流畅度。

### 测试

- 增加桌面端向上拖选并跨越视口边缘的真实浏览器回归，以及移动端选区手势的几何回归；验证选区、xterm 视口和外层历史投影始终同步。
- 增加六个常驻 PTY 同时断线重连、未变化快照复用、变化快照完整恢复、恢复期间实时帧排序和断线输入隔离回归；本机压力场景最大帧间隔从约 391ms 降至约 26ms，并通过 Web 全量单元测试、类型检查、静态检查、生产构建和相关浏览器端到端测试。

## [0.5.44] - 2026-08-19

### 修复

- 修复 Codex 在普通终端屏幕中把新内容推入历史时，xterm.js 未将局部滚动区移出的行写入 scrollback，导致 Dev Anywhere 向上回看时缺少部分系统记录的问题；兼容仅在 Codex 的完整同步输出事务与滚动、重绘位置全部吻合时启用，Claude、Shell、全屏滚动与 alternate screen 输出保持原样。
- 代理侧快照与发往 Web 的实时数据现在共同使用同一份兼容输出和连续序号，避免实时画面与重连快照对历史内容产生不同解释。

### 改进

- 移除 Web 端最多常驻三个 PTY 视图的 LRU 淘汰限制；所有访问过且仍存活的 PTY 会话都会保留 xterm 状态与订阅，会话结束或切换 Proxy 时仍会正常清理。

### 测试

- 增加 Codex 滚动区事务在任意数据分块、普通屏幕、alternate screen、全屏滚动和不完整事务下的确定性回归；使用现场保存的 9,404 帧重放，确认三次事务补回四行历史且底部可见画面完全一致。
- 增加四个以上 PTY 会话持续常驻且不被容量淘汰的 Web 回归，并通过 Proxy、Web 全量单元测试、类型检查和生产构建。

## [0.5.43] - 2026-08-18

### 修复

- 修复 Android 在 PTY 持续输出期间从历史回看向最新内容滑动时，画面可能停在冻结历史末端、下方出现空白，并继续保持回看状态而不再显示后续输出的问题；触摸或惯性滚动抵达冻结末端后，现在会退出回看、显示当前最新内容并恢复实时跟随。
- 保留用户向历史方向滑动时冻结当前画面、后台输出继续写入的原有行为；激活输入不再成为找回被错误冻结内容的必要操作。

### 测试

- 增加长 PTY 持续输出下触摸与惯性滚动抵达冻结末端的单元回归，并在真实 Android Chrome 模拟器中验证回到底后下一帧输出仍持续可见、无底部空白；同时验证静置实时输出不会自行进入回看。

## [0.5.42] - 2026-08-17

### 修复

- 修复桌面 Web 在 PTY 持续输出时先回看历史、再向下滚动，普通滚轮步进会被反复夹在旧画面末端、无法回到最新内容的问题；到达冻结回看画面的末端后，现在会直接恢复实时跟随，不再要求一次巨大的惯性滚动跨过持续增长的不可见间隔。
- 保留历史回看期间画面冻结、后台输出继续写入的原有行为；本次修复不改变 Android 触摸与软键盘收起手势的回看语义。

### 测试

- 增加长 PTY 历史持续输出时使用普通滚轮步进恢复实时尾部的确定性单测与真实 Chromium 回归，并复验 Android Chrome 下滑收键盘、历史冻结、显式回到底部、横屏和反复键盘开关流程。

## [0.5.41] - 2026-08-17

### 修复

- 修复桌面 Web 的短 PTY 会话在实时输出底部输入或删除字符时，xterm 内部自动滚动与外层虚拟滚动同时改写纵向位置、导致画面按整行上下抖动的问题；纵向跟随现在由 PTY 滚动控制器单独负责。
- 修复实时跟底与光标跟随在一次更新中分别读取和写入 `viewportY`、终端画布位置与容器滚动位置，可能让观察者短暂看到不同帧几何、继而放大为空白或抖动的问题；现在从同一不可变快照计算并原子提交一帧坐标。

### 测试

- 增加与现场桌面几何一致的真实 Chromium 输入回归，连续输入期间逐帧检查视口行与画布位置不变；加强 Android 软键盘历史回看与短 PTY 补画的用户可见内容、全屏覆盖和渲染就绪门禁，不再依赖底层 xterm 私有视口的偶然时序。

## [0.5.40] - 2026-08-15

### 修复

- 修复手机和平板切到后台或锁屏超过五秒后，即使原 WebSocket 仍然健康也会被无条件关闭重连、导致恢复前台时明显刷新和等待的问题；现在先在原连接上执行应用层 ping/pong 探活，只有连接失活或探活超时才换链重连。
- 兼容部分 Android Chrome 在应用进入后台时只发送窗口失焦/聚焦、不发送页面可见性事件的生命周期行为，确保健康连接继续使用，半开连接仍能在回到前台后恢复。
- 移除移动端 PTY 键盘辅助操作区上方的白色渐变阴影，减少终端内容上方的视觉干扰。

### 测试

- 增加真实 Android Chrome 后台恢复回归：页面持续接收输出时按 Home 停留超过五秒，健康连接必须保留同一页面、同一 socket 并继续更新；模拟失活连接则必须在探活超时后重连，同时保留原页面和输入状态。该流程在零 retry 下连续三轮通过，并完成局域网实体手机验收。

## [0.5.39] - 2026-08-15

### 修复

- 修复长 PTY 会话在最底部持续输出时误进入历史回看、画面像冻结一样不再跟随，以及切换会话、页面恢复或刷新后回放旧像素坐标，导致空白、轻微抖动、纵向或横向滚动失效的问题。页面重新激活时现在统一回到当前实时输出，不再保存与字体行高耦合的历史像素位置。
- 修复 Android 在 PTY 内向下滑动收起软键盘后，新释放的可见区域只有部分历史内容、顶部留下多行空白的问题；回看画面会按新视口完整重排，后续输出保持冻结，直到用户显式回到最新。
- 修复服务端固定行数的短 PTY 已在最底部时再向最新方向滑动，实时画布顶部暴露大块空白的问题；现在使用紧邻的真实历史行填满剩余可见区，不改变远端 PTY 行列几何。
- 修复慢速触摸回看被待处理输出向最新方向拉动一行、回到最新后仍遗留回看层，以及拖选文本的边缘自动滚动被跟底逻辑拉回的问题。

### 改进

- 将历史回看与短 PTY 补画收敛为单一的历史投影模型，删除多份快照缓存、字体像素恢复坐标和时间窗补偿状态；纵向回看意图只能由滚轮、触摸、自定义滚动条或选区自动滚动显式建立，不再从浏览器布局产生的 `scrollTop` 反推用户意图。

### 测试

- 增加 25 行真实 Android Chrome 软键盘下滑收起、短 PTY 底部额外滑动、慢速回看与持续输出、会话切换/后台/刷新、历史投影覆盖与选区边缘滚动回归；实机手势验收、Android 单模拟器零 retry 回归、桌面 E2E 和全工作区单元测试均通过。

## [0.5.38] - 2026-08-14

### 修复

- 修复 Android PTY 模式关闭软键盘后，原键盘区域保留大片空白、实时光标和有意义的终端内容未重新贴合可见区的问题；保持远端 PTY 行列几何稳定，不因浏览器键盘展开或收起向 TUI 发送额外尺寸变更。
- 修复 Chrome 原生恢复滚动位置时，外层容器与 xterm 已绘制视口可能使用不同帧的坐标，导致历史回看画面留在屏幕外或出现单行偏移的问题。
- 修复语音设置保存期间关闭面板后，延迟返回的能力请求仍可能更新已卸载界面的问题。

### 改进

- PTY 的 spacer、xterm viewport、host 位置、光标可见性与跟底判定统一使用实时有意义内容尾部锚点，并移除键盘关闭和图片预览旧修复留下的不可达分支与无用状态。
- 正式发布的 Android runner 固定使用专用模拟器，对设备和端口加入跨进程互斥，隔离 Vite 与 Relay，保留每个 spec 的独立失败产物，并在 reset、cleanup 或资源归属异常时立即失败。
- 质量检查先并行执行静态检查，再让真实计时器、WebSocket 和 jsdom 单测独占运行，避免嵌套并发把健康用例饿死为超时失败。

### 测试

- 增加键盘收起后实时内容占满可见区、浏览器原生滚动大跨度恢复、浮点行高边界、历史快照与画布真实可见几何的单测和端到端回归。
- 完整 Android Chrome 门禁在单模拟器、零 Playwright retry 下连续四轮通过，每轮 23 个文件、63 条用例；高风险桌面 PTY 与 Chaos 回归 39 条全部通过。

## [0.5.37] - 2026-08-13

### 修复

- 修复桌面 Web 的 PTY 模式中，从历史区域打开图片预览时仍会瞬间跳到最新输出的问题；终端应用启用焦点上报后，预览弹窗产生的失焦协议消息现在仍会正常发送给远端，但不再被误判为用户键入并触发跟底。
- 移除上一版本为图片预览增加的滚动位置保存与恢复补偿逻辑，避免关闭预览时二次改变阅读位置。
- 修复 Proxy 重启与 Relay 消息乱序同时发生时，较旧的空会话列表可能覆盖已重新注册的本地 PTY、导致会话暂时从 Web 列表消失的问题。

### 测试

- 更新真实桌面 Chromium 回归测试，显式启用终端焦点上报，并逐次检查预览打开、加载完成和关闭期间的 PTY 滚动位置；同时验证焦点协议消息仍会送达远端，现有图片预览与文件下载链路保持可用。

## [0.5.36] - 2026-08-13

### 修复

- 修复桌面 Web 的 PTY 模式中，从历史区域 Cmd/Ctrl 点击图片路径打开预览时，弹窗引起的焦点、布局或终端重绘可能把阅读位置强制拉到最新输出的问题；预览期间现在会保护并在关闭时恢复原历史锚点。

### 测试

- 补充真实桌面 Chromium 回归测试，覆盖长 PTY 历史中的图片预览、加载期间新输出与布局变化，并继续验证文件下载不会改变历史滚动位置。
...
- Chat 模式助手输出改为带 `turnId` 和 `revision` 的版本化全文消息；刷新、重连、重复或乱序传输时以前端收到的最新全文覆盖旧版本，避免只显示残缺分片。
- 正式发布恢复进程与网络 Chaos 硬门禁，并继续强制串行运行完整 Android Chrome E2E；真实文件上传下载链路保持按需执行。
- 移动端测试固定使用单模拟器和软件 GPU，改进设备清理、键盘状态恢复与分阶段耗时记录，降低模拟器资源竞争造成的随机失败。

### 修复

- 修复桌面 Chrome 切换到其他标签页超过五秒后，页面恢复可见时主动关闭仍然有效的 Relay WebSocket 并显示重连的问题；强制换链仅保留给手机和平板的锁屏与长后台恢复场景。
- 修复 Chat 模式刷新或恢复前台时，已落盘历史覆盖同期实时回复、留下陈旧半截气泡或产生重复消息的问题。
- 修复 PTY 中 Cmd/Ctrl 点击文件路径触发下载时意外恢复跟底，以及硬换行路径被拼接成错误超长链接的问题；路径中的空格不再作为普通自动识别规则。
- 修复移动端 Chat 软键盘遮挡、发送后未定位到最新消息，以及 PTY 键盘、滚动、选区与横向视图恢复中的多项边缘问题。

### 测试

- 完整 Android Chrome 门禁连续八轮通过；完整 Chaos 门禁覆盖 Relay、Proxy、Web、PTY、Claude/Codex worker 重启与乱序恢复并通过。
- 补充全文消息版本覆盖、刷新历史合并、桌面与移动端 WebSocket 后台恢复、PTY 链接点击及移动端真实键盘行为的回归测试。

## [0.5.33] - 2026-08-11

### 修复

- 修复历史 `apply_patch` 工具详情只显示原始补丁文本、缺少实时工具调用已有的红绿 Diff 行、行号和增删统计的问题；历史补丁现在按文件拆分并复用统一的 Diff 预览组件。

### 测试

- 补充历史补丁文本归一化单元测试与桌面浏览器 E2E，并使用真实会话的 116 个 Patch 活动确认 198 个详情全部进入 Diff 展示，无空详情或普通文本降级。

## [0.5.32] - 2026-08-10

### 修复

- 修复 Codex 历史工具活动把外层 JavaScript 编排脚本直接显示为 Shell 命令的问题；现在会恢复真实命令、等待进程、终端输入、图片查看、补丁、计划和 Web 操作的语义化摘要与参数。
- 补充批量命令、批量图片、变量参数、对象属性简写及损坏旧记录的安全降级，无法完整还原的记录不再冒充 Bash 命令或暴露内部编排脚本。

### 测试

- 补充 Codex 单条、批量、变量引用、旧格式及异常格式历史解析回归测试，并全量扫描本机 360 个 Codex 会话（18.06 GB、195,347 条工具调用）与 29 个 Claude 会话，确认没有 JS 包装误标、非空未识别 `exec`、参数丢失或状态错配。

## [0.5.31] - 2026-08-10

### 修复

- 修复 Codex 历史记录中的原始 `custom_tool_call` 命令只显示“运行命令”、无法展开查看具体命令参数的问题。
- 修复 Docker 部署把 Service Worker、注册脚本和 Web Manifest 误设为长期不可变缓存，导致桌面或移动浏览器升级后仍运行旧版 Web 界面的问题。

### 测试

- 补充 Codex 原始命令历史解析与 Docker 同构 Web 资源目录缓存策略回归测试，并使用真实会话确认历史命令参数完整恢复。

## [0.5.30] - 2026-08-10

### 改进

- 聊天模式中的单张图片改为紧凑缩略图，点击后仍进入完整预览；多图继续使用横向 Gallery。
- 历史消息中的工具调用恢复命令、补丁和通用参数详情，与实时工具活动保持一致。

### 修复

- 修复 Chat 模式在手机上从历史位置主动发送消息后没有定位到最新气泡的问题；助手输出和其他端同步消息仍不会打断历史阅读。
- 修复部分 Android 键盘布局下聊天输入卡与键盘工具栏间距不足的问题；键盘覆盖避让与可见安全间距现在分别计算。
- 修复 PTY 会话切换浏览器标签页或进入后台后自动跳到底部的问题；原本位于底部时继续跟随，正在回看时恢复原位置并提示后台新输出。

### 测试

- 补充主动发送与被动新消息的滚动策略、PTY 后台恢复、历史工具详情和紧凑图片预览回归测试。
- 通过 2077 项自动化测试、桌面浏览器 E2E、类型检查、代码检查和完整生产构建。

## [0.5.29] - 2026-08-10

### 新增

- 用户发送的图片在聊天气泡内直接显示为横向 Gallery；多图支持手机横滑、下一张预露出和动态边缘渐隐，并始终裁切在气泡边界内。
- 非图片上传改为语义化文件卡片，只显示文件类型和下载操作，不再在用户消息中暴露文件名或本地路径。

### 修复

- 修复手机锁屏或长时间切到后台后，Chat 模式保持旧连接和旧消息、必须手动刷新才能看到最新内容的问题；恢复前台时会重建疑似半开连接并与服务端历史重新对齐，同时保留草稿和排队消息。
- 修复手机后来打开电脑上正在工作的 Chat 会话时，历史记录缺少工具调用的问题；Claude 与 Codex 的历史工具活动现在会恢复运行中、成功和失败状态。

### 测试

- 补充后台恢复连接、历史消息竞态对齐、附件后缀识别、Gallery 边界渐隐、文件卡下载、Claude/Codex 历史工具解析及共享协议往返测试。
- 通过 2074 项自动化测试、类型检查、代码检查和完整生产构建。

## [0.5.28] - 2026-08-10

### 修复

- 修复 Android 聊天模式中软键盘工具栏遮挡输入框、附件按钮和发送按钮的问题，聊天输入区现在与 PTY 控件共用键盘防遮挡间距。
- 修复移动端聊天消息、系统标记和超长连续路径可能横向撑出视口的问题，同时保持换行后的文件下载与图片预览链接可点击。
- 移除聊天模式 Diff 预览中重复的“旧 / 新 / 内容”列头，保留文件标题、增删统计、行号和变更标记。
- 统一运行中与已完成工具活动圆点的状态槽尺寸，使闪烁和静态圆点保持在同一条垂直轴线上。

### 测试

- 补充真实 Android Chrome 的聊天键盘避让、长气泡边界和换行路径原生点按下载回归测试。
- 补充 Diff 结构与活动状态圆点浏览器几何回归，并通过全量 Web 单元测试、桌面 E2E、代码检查和生产构建。
...
- 修复下载中文文件名时，Relay 因响应头包含非 ASCII 字符而退出、页面误报 Relay 断连的问题。
- 下载响应元数据异常时只终止当前文件流，不再影响 Relay 与其他会话。

### 测试

- 补充中文文件名的真实 HTTP 流式下载集成测试，并通过 Relay 全量测试、类型检查和生产构建。

## [0.5.21] - 2026-07-27

### 修复

- 修复手机端从 PTY 底部慢慢上滑时，待处理的新输出与重排可能把画面向底部拉回一整行，造成滚动抖动的问题。
- 输入长命令时，光标会在接近屏幕右侧前提前触发横向跟随并回到视野中部，不再等到越过边缘后才调整画面。

### 测试

- 补充 Android Chrome 慢速触摸回看回归测试，覆盖 `80×29` 终端、五千行历史和持续原地更新。
- 补充横向光标提前跟随的单元测试与 Android Chrome E2E，并通过完整 Web 单元测试、桌面 PTY E2E、类型检查、代码检查和生产构建。

## [0.5.20] - 2026-07-27

### 修复

- 修复 PTY 进入回看状态后，即使最新输出仍在屏幕内，`Working` 计时等原地更新内容也可能停止刷新的问题。
- 回看期间会保持用户的滚动位置：最新输出仍可见时继续实时刷新，离开可视区后再冻结历史画面。

### 测试

- 补充 Android Chrome 回看边界的实时更新回归测试，并在真实 Codex 会话中验证计时更新不会改变滚动位置。
- 通过 PTY 滚动单元测试、桌面 Chromium 回看 E2E、Android Chrome 模拟器 E2E 和生产构建。

## [0.5.19] - 2026-07-27

### 修复

- 修复手机上新建 PTY 会话后，普通或快速上滑回看历史时画面可能跳动、底部短暂缺失最后一行的问题。
- PTY 回看滚动现在会在切换 xterm 视口前准备完整的目标画面，并保留一行视觉余量；不会改变会话实际的终端行列数。

### 测试

- 补充 Android Chrome 新建终端会话的逐帧滚动回归测试，覆盖普通拖动、快速滑动、末行覆盖与画面几何连续性。
- 在桌面 Chromium 和 iPad Safari 布局环境中验证 PTY 历史回看与既有会话尺寸未受影响。

## [0.5.18] - 2026-07-27

### 修复

- 修复 PTY 回看历史期间，后台持续输出可能触发历史画面重新渲染，导致用户正在查看的内容发生变化的问题。

### 测试

- 补充无纵向位移的被动滚动事件回归测试，并在桌面 Chromium 与 Android Chrome 模拟器中验证持续输出期间的历史画面保持稳定。

## [0.5.17] - 2026-07-26

### 改进

- 从浏览器新建 PTY 会话时，会根据桌面、iPad 或手机的可用空间确定初始行列数，减少终端下方的大面积空白；初始尺寸不会低于 `80×24`，避免二维码、表格和常见 TUI 因终端过窄而损坏。
- 用户上滑回看 PTY 输出后，当前可见画面会作为一个完整帧保持稳定；后台输出继续接收，只有用户继续滚动时画面才整体推进，不再出现历史区静止、当前区单独更新的割裂效果。
- 复用现有的“回到底部”按钮提示新输出，有新内容时按钮会显示“回到最新”，滑到底部或点击按钮后恢复实时跟随。

### 测试

- 补充初始 PTY 行列数的单元测试及手机、iPad、桌面布局 E2E，覆盖 `80×24` 下限、宽屏扩展和既有会话尺寸兼容。
- 补充跨越历史区与当前屏边界的持续输出 E2E，验证回看画面冻结、继续滚动后的整帧更新，以及回到底部后的实时跟随。

## [0.5.16] - 2026-07-25

### 修复

- 保存 Relay Token 后会明确提示保存结果，并自动重连 Relay、重新拉取开发机列表，不再需要用户手动刷新。
- 浏览器存储不可用时会提示保存失败，且不会误报成功或使用未保存的 Token 发起重连。

### 测试

- 补充 Relay Token 保存成功、存储失败的单元测试，以及保存后重新请求开发机列表的桌面浏览器 E2E。

## [0.5.15] - 2026-07-25

### 新功能

- 聊天会话上传图片或文件后，会在输入框上方显示缩略图或文件卡片，不再把开发机临时路径直接塞进输入内容；附件可以在发送前单独移除。
- 长图和宽图会在固定尺寸卡片中完整等比缩放；多个图片与文件混合上传时保持单行排列，可横向滑动查看。

### 改进

- 首页打字机品牌中的 `DEV` 改为与侧边栏一致的琥珀色。
- 附件区域会随内容自然撑高输入区并压缩聊天内容空间，移除全部附件后自动恢复，不会覆盖聊天气泡。

### 测试

- 补充图片粘贴、文件上传、移除附件及发送载荷保持原有 `@路径` 语义的桌面 E2E 和单元测试。
- 在 Android Chrome 模拟器中验证图片与文件混合、长宽图缩放、多图横向滚动，以及附件出现和移除时聊天区与输入区的布局恢复。

## [0.5.14] - 2026-07-25

### 修复

- PTY 尺寸改由会话端稳定持有，手机刷新、重连、旋转屏幕或唤起软键盘时不再隐式改变真实终端尺寸，避免二维码、表格和历史输出在窄屏上被重新折行。
- 窄屏继续通过横向滚动查看完整终端画布，字号设置只调整浏览器中的显示尺寸，不影响远端 Shell 或 coding agent 会话。
- 已有会话无需迁移；即使会话列表没有尺寸元数据，也会继续从既有终端快照恢复原始行列数。

### 测试

- 补充旧会话无尺寸元数据、视口变化不发送终端 resize、宽行不重排的桌面与 iPad Safari E2E。
- 在真实 Android Chrome 模拟器中验证 Shell 会话重连后保持服务端尺寸、二维码模拟行不折行、横向触摸滚动及输入跟随。

## [0.5.13] - 2026-07-24

### 修复

- Android 上的 PTY 键盘辅助区现在会在防覆盖净空之外额外保留可见间距，避免部分系统输入法吃掉全部底部留白后，辅助按键再次紧贴键盘边缘。
- 移动端图片预览操作区现在会主动避开系统手势区域，同时保持预览图片完整可见。

### 新功能

- 图片预览新增“复制图片”操作，图片会在浏览器中规范化为 PNG 后写入系统剪贴板。

### 测试

- 加强竖屏双行、横屏单行和真实 Android 软键盘的布局断言，分别验证键盘覆盖保护与可见间距。
- 补充图片剪贴板单元测试，以及移动端图片预览操作区的底部净空、等宽按钮和无覆盖布局回归测试。

## [0.5.12] - 2026-07-23

### 新功能

- VPS Relay 现在既可以使用域名，也可以直接使用公网 IPv4 地址部署；安装脚本会申请 HTTPS 证书并配置自动续期。

### 改进

- 切换连接较慢的开发机时，会立即显示目标开发机和连接进度，不再继续展示上一台开发机的会话、文件和 coding agent 状态。
- 重新整理中英文 README 与部署文档，明确本地 coding agent 会话接管、远程新建会话、Quick Tunnel、VPS Relay、公网 IP 部署、跨设备支持和 Voice Pilot 的使用方式。

### 测试

- 补充开发机慢速切换的桌面与移动视口 E2E，以及跨开发机异步响应隔离的状态回归测试。
- 在真实 VPS 上验证公网 IP 证书签发与续期、HTTPS、WSS、Proxy 连接、浏览器访问及重复执行安装脚本。

## [0.5.11] - 2026-07-21

### 修复

- 修复移动端 PTY 键盘辅助区紧贴系统软键盘边缘，导致底排按键可能与输入法工具栏交叠的问题；横屏单行和竖屏双行布局现在都会为可点击区域保留稳定净空。

### 测试

- 补充辅助键区底部净空回归测试，并在 Android 模拟器中实际旋转横竖屏、唤起系统键盘验证，同时覆盖 iPad 横屏布局。

## [0.5.10] - 2026-07-19

### 修复

- 修复 PTY 会话正在输出时，画面偶尔停留在旧内容，直到用户点击、滑动或输入后才一次性恢复的问题。现在即使用户正在回看历史输出，新数据也会持续写入终端，同时保留当前查看位置。

### 测试

- 补充 PTY 回看期间持续输出与视口保持的回归测试，并在桌面 Chromium、Android Chrome 和 iPad Safari 布局中验证。

## [0.5.9] - 2026-07-19

### 修复

- 修复上传视频或较大文件时，大量传输数据可能延迟心跳响应并被误判为 Relay 断线，继而导致上传中断的问题；收到有效数据或控制消息现在同样会确认连接仍然存活。
- VPS 的远程上传接口改为流式转发并使用长连接超时，避免耗时上传在约一分钟后被反向代理提前终止。
- 上传取消或写入失败后会删除开发机上的未完成文件，避免留下无法使用的临时文件。

### 测试

- 补充上传期间连接活性、取消上传清理和 VPS 反向代理配置的回归测试。

## [0.5.8] - 2026-07-19

### 修复

- 修复 iPad Safari 的通用文件选择器将 ZIP、数据库及其他非图片文件置灰，导致无法上传的问题。
- “上传图片”扩展为“上传照片或视频”，系统相册现在会显示并允许上传录屏等视频文件。
- Android 的通用文件入口继续使用文档选择器，避免部分浏览器错误触发相机权限请求。

### 测试

- 补充各平台上传选择器策略、视频跳过静态图片压缩和桌面上传菜单的回归测试，并在 Android Chrome 原生文件选择器中验证文档上传入口。

## [0.5.7] - 2026-07-19

### 修复

- 将页面刷新或手机浏览器唤醒后的重连宽限期从 10 秒延长至 30 秒，减少慢网络下连接尚在恢复就过早退回连接页的问题。
- 历史会话恢复请求同步使用 30 秒超时；超时提示改为“可能遇到问题”的暂态说明，并明确页面仍在等待开发机返回，避免把迟到但仍会成功的响应误报为确定失败。
- 重连遮罩改用非确定性文案，并明确 DEV Anywhere 会继续尝试恢复当前会话。

### 测试

- 补充 30 秒重连宽限期和历史会话恢复提示的状态机测试，并在桌面 Chromium 与 Android Chrome 模拟器中验证异常态布局和文案。

## [0.5.6] - 2026-07-19

### 修复

- 修复移动端长按 PTY 中较短的英文或技术标识符时，初始选区会把相邻中文或其他词语一并选中的问题；现在按自然语言词边界选择，同时保持命令参数、技术标识符和文件路径完整。

### 测试

- 补充中文分词、中英无空格混排与技术标识符回归测试，并在 Android Chrome 的真实 xterm 链路中验证长按选择、复制、路径操作和选区手柄。

## [0.5.5] - 2026-07-18

### 修复

- 修复 Codex 等终端程序使用框线字符绘制启动横幅或表格时，字体分片晚于终端首帧加载导致边框错位的问题。

### 改进

- 使用修复后的真实 Codex 会话重新录制 README 中的新建会话与审批、搜索、文件下载演示。

### 测试

- 补充终端框线字体分片预加载与终端初始化顺序测试，并在真实 Codex PTY 中验证表格对齐。

## [0.5.4] - 2026-07-18

### 修复

- 修复 PTY 会话已完成任务后，终端光标或界面重绘输出可能再次将会话标记为“工作中”，并导致状态无法自动恢复为空闲的问题。

### 测试

- 补充 PTY 任务完成后发生终端重绘的状态回归测试，并使用真实 Codex PTY 验证连续任务与空闲期重绘。

## [0.5.3] - 2026-07-18

### 改进

- 静态大图在上传前会于浏览器内自适应缩放并压缩；文件选择、拖放和粘贴图片均使用同一处理链路，压缩无收益或处理失败时仍会上传原图。

### 修复

- 修复手机浏览器从后台唤醒并重连时，旧连接上的历史会话请求失败会提前提示“无法加载历史会话”，但新连接随后又能正常恢复的问题；重连中的失效请求不再误报，连接稳定后的真实加载失败仍会提示。
- 修复 Android 软键盘展开并压缩聊天视口后，当前搜索结果仍停留在键盘遮挡区域的问题；视口尺寸变化时会重新定位当前命中项。
- 修复刷新或从后台恢复 PTY 页面时，浏览器恢复旧滚动位置导致终端落在顶部或最右侧的问题；PTY 现在统一接管恢复位置并回到底部行首。

### 测试

- 补充历史会话同步在稳定连接失败与重连期间失败两种场景的回归测试。
- 补充静态大图上传压缩测试，并在桌面 Chromium、Android Chrome 与 iPadOS 26.5 Safari 上验证上传链路和浏览器原生图片编码。

## [0.5.2] - 2026-07-17

### 修复

- 修复 Android Chrome 弹出软键盘后，PTY 辅助输入区可能在单行与双行之间反复跳变的问题。
- 修复移动端拖动 PTY 选区到屏幕边缘自动滚动时，选区可能被提前清除、操作手柄消失的问题。

### 测试

- Android 模拟器现在通过系统无障碍节点执行真实点按，软键盘、辅助输入、粘贴和长按复制用例不再因键盘未弹出而跳过。
- 补充辅助输入区高度稳定性和选区边缘自动滚动回归测试，并在 iPadOS 26.5 Safari 模拟器中验证真实 PTY 与聊天会话布局。
