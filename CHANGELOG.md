...
- 完全重写中英文 README，以中文作为默认入口，重新组织项目介绍、快速体验、VPS 部署、核心功能、跨设备支持与安全边界。
- 使用真实 DEV Anywhere 会话重录桌面端、PTY、聊天模式、工具审批、文件操作、Voice Pilot 和跨设备访问的截图与动图。
- 精简 `docs` 目录，移除过时的计划、审计、故障记录和重复说明，保留重新整理后的 VPS 部署与开发指南。

### 修复

- 修复从 Web 新建 Codex 聊天会话时原生 thread ID 可能早于会话发布到达，导致刷新后缺少历史消息的问题。
- 修复 Relay 从隐藏目录或包管理器存储路径托管 Web 资源时首页无法正确返回的问题。

### 测试

- 补充 Codex 原生会话 ID 在 worker 就绪与会话发布之间传递的单元测试，以及 Relay 隐藏资源目录的集成测试。

## [0.5.0] - 2026-07-17

### 新功能

- Voice Pilot 完成可用性升级：支持连续语音对话、回复播报、语音审批、复述与语音退出，并增加启停提示音、实时波形和运行状态。
- Agent 气泡聊天与 PTY 终端均支持会话内搜索：桌面端可使用 `Cmd/Ctrl + F`，移动端可从会话菜单进入，并可在匹配项之间定位。
- 新增可选的会话空闲浏览器通知，当后台会话从工作中转为空闲时提醒用户。
- 新增免账号 Quick Tunnel 体验模式，可通过 Cloudflare 随机域名快速体验 DEV Anywhere；正式部署仍推荐自建 VPS Relay。

### 改进

- Relay 现在直接托管 Web 应用，统一镜像、端口与部署流程，不再需要独立 Web Nginx 容器。
- 重新整理中英文 README，使用真实产品截图介绍 Agent 与终端会话、移动端操作和客户端管理。
- 完善 Agent 权限模式的新建、恢复与提示流程，统一 Claude Code 和 Codex 会话的操作语义。
- 重新整理 iPad 设置、对话框、辅助输入区、软键盘间距与横竖屏布局，并补充浮动键盘提示。

### 修复

- 修复 iPad Safari 在软键盘、实体键盘、横竖屏切换、PTY 聚焦和页面滚动下的遮挡、空白区、焦点漂移与整页误滚动问题。
- iPad 端当前以 Safari 为正式支持浏览器；使用 Chrome、Firefox 等其他浏览器时显示明确引导，避免 iPadOS 系统输入工具条导致终端不可用。
- Voice Pilot 重构录音、VAD、ASR、TTS、音频会话与状态机边界，修复首次准备过慢、空识别、底噪误触发、连接中断、播报无声、未完成回复时提前聆听和 URL 被朗读等问题。
- Voice Pilot 在锁屏或进入后台时会安全暂停并释放麦克风、播放、连接与屏幕常亮资源，回到前台后由用户明确继续。
- 修复语音审批对“不同意”等否定表达的匹配顺序，非审批句不再在等待审批时被误执行。
- 修复 Agent 会话启动失败后的 worker、PTY 和 UI 残留，并补强终止与恢复流程。
- 修复 PTY 文本选中菜单在滚动后仍停留原位，以及窄屏下跨行文件路径识别与操作问题。

### 测试

- 新增 iPad 0.5 验收清单，在真实 iPad Safari 上完成软/硬键盘、横/竖屏、PTY、气泡聊天、Agent 权限、Voice Pilot、锁屏恢复和回声防护验收。
- 新增可固定录音输入的 Voice Pilot 真实链路 E2E，覆盖 Wake Lock、VAD、ASR、Agent 回复与 TTS。
- 扩充会话搜索、空闲通知、Agent 权限、会话清理、iPad 布局和移动端交互的单元、集成与 Playwright 回归测试。

## [0.4.84] - 2026-07-01

### 修复

- 调整移动端 PTY 辅助输入栏显示条件：必须在软键盘实际打开后才显示，避免仅聚焦终端时提前挤占底部空间。

### 测试

- 更新移动端 PTY 辅助输入栏单测与 Android E2E，覆盖「已聚焦但键盘未打开时不显示」的回归场景。

## [0.4.83] - 2026-07-01

### 修复

- 修复长 PTY 会话首帧快照超过 1MiB 时被 Relay 丢弃，导致页面一直停在「正在连接终端」或「同步中断」的问题；Relay 和 Proxy 现在共享同一套 WebSocket JSON 消息上限。

### 测试

- 补充超过 1MiB 的 PTY `session_snapshot` 端到端路由回归测试，确保大快照能从 Proxy 经 Relay 到达浏览器客户端。
...
- 修复 PTY 等待审批提示条覆盖终端内容、视觉上贴近标题栏的问题；提示条现在位于标题区下方的正常布局流中。
- PTY 等待审批时不再同时显示 4px 状态线和审批提示条，避免两个 UI 元素重复表达同一状态。
- 审批提示条保留低频背景呼吸效果，但文字和 `Always yes` 按钮保持稳定，避免整条提示反复闪烁。
- 修复 PTY 文件下载和图片预览候选路径误触发无效链接的问题；Relay 现在会先让开发机按当前会话目录预检路径是否存在且为普通文件，预检通过后才发放临时 URL。
- 移动端开发机选择页在慢连接下增加即时连接反馈，点击开发机后会显示「正在连接」并临时禁用其它选项。

### 测试

- 补充 PTY 审批提示条布局与动画回归断言，确保提示条不再以 overlay 形式覆盖终端区域。
- 补充远程文件 URL 预检、PTY 相对路径下载和慢连接开发机选择反馈回归测试。

## [0.4.80] - 2026-06-26

### 修复

- 修复 PTY 图片预览和文件下载对 `~/...` 路径的处理：前端不再把 home 路径截成 `/...`，Proxy 读取远端文件时会按开发机 home 目录展开 `~`，避免本机存在的图片仍提示无法预览。

### 测试

- 补充图片预览、文件下载和 Proxy 远端文件解析对 `~/...` 路径的回归测试。

## [0.4.79] - 2026-06-21

### 改进

- 将「回到底部」按钮移动到右上角，PTY 视图使用更靠下的顶部偏移，减少对底部操作区的遮挡。

### 修复

- 修复移动端在键盘未弹起时长按 PTY 内容会误聚焦终端输入框的问题，避免软键盘弹出后选区和复制工具条漂移。
- 修复 PTY 专用 E2E fake Relay 缺少 client auth preflight mock 导致移动端 PTY 测试卡在「中继连接已中断」的问题。

### 测试

- 新增 PTY 长按不会聚焦输入框的 Android 模拟器回归测试，并补充 PTY focus suppression、tap 恢复输入焦点和回到底部位置测试。

## [0.4.78] - 2026-06-20

### 修复

- 修复图片预览在长路径和底部操作区下可能横向溢出的问题，并区分「获取预览链接」与浏览器实际图片加载状态，避免图片尚未解码时提前显示「图片已加载」。
- 修复移动端打开超宽图片时没有按视口 fit、右侧被裁切且无法横向移动的问题；预览现在按图片自然尺寸和可视区域计算初始缩放。

### 测试

- 补充图片预览加载状态单测，以及长路径布局、移动端超宽图 fit、缩放/拖拽/双击重置的 Playwright 回归测试。
- 补齐 E2E fake Relay 的 client auth preflight mock，避免当前连接流程在测试中卡在「连接中」。

## [0.4.77] - 2026-06-19

### 修复

- 将远程文件下载、图片预览和上传的临时链接有效期从 5 分钟延长到 24 小时，避免大文件下载或浏览器延迟拉起时链接过期。

### 测试

- 补充 Relay 下载/上传临时链接有效期回归断言，并跑通 Relay 集成测试与类型检查。

## [0.4.76] - 2026-06-17

### 修复

- 修复历史会话扫描提前停止读取时未关闭底层文件流的问题，避免长期运行后 `.jsonl` 文件描述符泄漏导致创建终端会话失败。

### 测试

- 新增 Codex 历史扫描重复执行时文件描述符不增长的回归测试。
