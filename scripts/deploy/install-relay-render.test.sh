...
assert_contains "$nginx" "location ~ ^/(proxy|proxy-stream|client|voice/asr|voice/tts)$"
...
assert_contains "$nginx" "location ^~ /api/device-preview-streams/"
assert_contains "$nginx" "proxy_cache off;"
