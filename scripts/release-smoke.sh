...
bash scripts/web-e2e.sh --base-url http://localhost:5173 -- e2e/pty-smoke.spec.ts e2e/clipboard-image.spec.ts e2e/image-preview.spec.ts --project=device-pc
DEV_ANYWHERE_REAL_CLIPBOARD_IMAGE_SMOKE=1 \
  WEB_BASE_URL=http://localhost:5173 \
  pnpm --filter @dev-anywhere/web run test:e2e -- e2e/real-clipboard-image.spec.ts --project=device-pc
