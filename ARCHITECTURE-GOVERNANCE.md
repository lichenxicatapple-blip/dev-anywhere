...
- web 侧同一个 `session.state` 同时接收 `session_status` 和 `pty_state` 映射，说明 `pty_state` 仍在参与主状态路径。
...
4. **降级 `pty_state`**：`pty_state` 保留给短期 UI 提示或调试，不再写入 `session.state` 主路径。
...
4. web 增加 agent status store/dispatcher，status line 改读 agent status + lifecycle 聚合。
5. 删除 web `pty_state -> session.state` 主路径映射。
