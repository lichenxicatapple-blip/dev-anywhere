...
1. 在开发机上保持 `dev-anywhere serve start --env cloud` 运行。
...
5. 不希望这台机器继续通过 Relay 暴露时，运行 `dev-anywhere serve stop` 停掉本地 daemon。
...
开发完整的本地 proxy/relay/web 链路时，可以使用 `pnpm dev:restart` 和 `pnpm dev:health`。
