...
test.describe("PTY browser smoke", () => {
  ...
  test("does not pin users to bottom for mouse-reporting clicks while reviewing history", async ({
    page,
  }) => {
    await installFakeRelay(page);
    await page.goto(`${BASE_URL}/#/chat/${SESSION_ID}?mode=pty`);
    ...
    const terminal = page.locator('[data-slot="pty-terminal"]');
    await page.evaluate(() => {
      window.__ptySmoke.sendPty("\x1b[?1000h\x1b[?1006h");
      window.__ptySmoke.sendPty(
        Array.from(
          { length: 120 },
          (_, i) => `mouse mode history ${String(i).padStart(3, "0")}\r\n`,
        ).join(""),
      );
    });
    await expect
    ...
    await expect(page.locator('[data-slot="back-to-bottom"]')).toBeVisible();
    const before = await terminal.evaluate((el) => {
      const node = el as HTMLElement;
      return {
        scrollTop: node.scrollTop,
    ...
    const point = await page.evaluate(() => {
      const screen = document.querySelector<HTMLElement>('[data-slot="pty-host"] .xterm-screen');
      const metrics = window.__ccTest?.pty.metrics("pty-smoke");
      ...
      return {
        x: rect.left + cellWidth * 20,
        y: rect.top + cellHeight * 8,
      };
    });
    expect(point).not.toBeNull();
    await page.mouse.click(point!.x, point!.y);
    await expect.poll(() => readRawPtyInput(page)).toContain("\x1b[<");

    const after = await terminal.evaluate((el) => {
      const node = el as HTMLElement;
      return {
        scrollTop: node.scrollTop,
        bottomGap: node.scrollHeight - node.clientHeight - node.scrollTop,
    ...
    expect(after.scrollTop).toBeLessThanOrEqual(before.scrollTop + 8);
