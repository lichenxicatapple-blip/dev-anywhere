...
import {
  adjustPtyGeometry,
  measurePtyFitGeometry,
  type PtyGeometry,
  type PtyResizeAction,
} from "@/lib/pty-fit-geometry";
...
interface UsePtyViewResult {
  ...
  clearFind: () => void;
  resizeTerminal: (action: PtyResizeAction) => void;
...
export function usePtyView(options: UsePtyViewOptions): UsePtyViewResult {
  ...
  const pendingResizeRef = useRef<PtyGeometry[]>([]);
  ...
  const resizeTerminal = useCallback(
    (action: PtyResizeAction): void => {
      ...
      // Count every click even while earlier resize events are still in transit.
      const current = pendingResizeRef.current.at(-1) ?? terminal;
      const geometry =
        action === "fit"
          ? measurePtyFitGeometry(containerEl, host, terminal)
          : adjustPtyGeometry(current, action);
      if (!geometry) {
        toast.error("无法获取终端显示区域的尺寸");
        return;
      }
      if (geometry.cols === current.cols && geometry.rows === current.rows) {
        return;
      }
      ...
      if (!relay.sendControl({ type: "terminal_resize_request", sessionId, ...geometry })) {
        ...
        pendingResizeRef.current.push(geometry);
        useSessionStore.getState().setPtyGeometry(sessionId, geometry);
      }
    },
  ...
  useEffect(() => {
    if (!active || !connection.ready || !connected || !proxyOnline) pendingResizeRef.current = [];
    const current = pendingResizeRef.current.at(-1) ?? terminalRef.current;
    useSessionStore
      .getState()
      .setPtyGeometry(
        sessionId,
        connection.ready && connected && proxyOnline && current
          ? { cols: current.cols, rows: current.rows }
          : null,
      );
  }, [active, connection.ready, connected, proxyOnline, sessionId]);
  ...
  useEffect(() => {
    ...
    let resizeRegistration: { dispose(): void } | null = null;
    ...
    const termCtrl = attachPtyTerminalController({
      ...
      onTerminalReady: (term) => {
        ...
        resizeRegistration = xterm.onResize(({ cols, rows }) => {
          const pending = pendingResizeRef.current;
          const index = pending.findIndex((size) => size.cols === cols && size.rows === rows);
          // Retire acknowledged requests; a different size came from another viewer or a snapshot.
          pendingResizeRef.current = index >= 0 ? pending.slice(index + 1) : [];
          if (readyRef.current && transportAvailableRef.current) {
            useSessionStore
              .getState()
              .setPtyGeometry(sessionId, pendingResizeRef.current.at(-1) ?? { cols, rows });
          }
        });
      ...
      onReady: () => {
        ...
        const terminal = terminalRef.current;
        if (terminal) {
          useSessionStore.getState().setPtyGeometry(sessionId, {
            cols: terminal.cols,
            rows: terminal.rows,
          });
        }
        connection.transport.onReady();
      },
    });
    ...
    return () => {
      ...
      pendingResizeRef.current = [];
      useSessionStore.getState().setPtyGeometry(sessionId, null);
