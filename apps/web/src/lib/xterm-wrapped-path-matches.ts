...
export type XtermPathMatch = {
  path: string;
  startColumn: number;
  endColumn: number;
};

export type XtermBufferPathSegment = {
  lineNumber: number;
  startColumn: number;
  endColumn: number;
};

export type XtermBufferPathMatch = {
  path: string;
  startLineNumber: number;
  startColumn: number;
  endLineNumber: number;
  endColumn: number;
  segments: XtermBufferPathSegment[];
};

type PathExtractor = (text: string) => string[];
...
export function findXtermPathMatchesInWrappedBuffer(
...
function findPathSpans(line: string, extractPaths: PathExtractor): PathSpan[] {
  const paths = extractPaths(line);
  const matches: PathSpan[] = [];
  let searchFrom = 0;
  for (const path of paths) {
    ...
    matches.push({ path, startIndex: atIndex, endIndex });
...
function getWrappedLineBlock(
  ...
  for (let guard = 0; guard < MAX_WRAPPED_PATH_LINES; guard += 1) {
...
function getPathSegmentsForSpan(
  ...
  startIndex: number,
  endIndex: number,
): XtermBufferPathSegment[] {
  const segments: XtermBufferPathSegment[] = [];
